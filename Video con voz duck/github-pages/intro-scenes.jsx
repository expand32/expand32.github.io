...
const TRACK_SRC = 'assets/expand32-intro-mix.wav';
...
// ── Now-playing widget ──
// Hidden per-scene <video> carrying the export-audio contract: each scene
// contributes its exact slice of the track, so the exported file has sound.
let _starts = null;
function sceneStarts() {
  if (_starts) return _starts;
  try {
    const sc = JSON.parse(window.OM_SCENES);
    const st = [0];
    sc.forEach((s, i) => st.push(st[i] + s.dur));
    _starts = st;
  } catch (e) { _starts = [0]; }
  return _starts;
}
function TrackForExport() {
  const sc = useScene();
  const ref = React.useRef(null);
  const starts = sceneStarts();
  const s0 = starts[sc.index] || 0;
  const s1 = starts[sc.index + 1] != null ? starts[sc.index + 1] : s0 + sc.dur;
  React.useEffect(() => {
    const v = ref.current;
    if (!v || v.readyState < 1) return;
    const target = s0 + Math.min(sc.localTime, sc.dur);
    if (Math.abs(v.currentTime - target) > 0.25) { try { v.currentTime = target; } catch (e) {} }
  });
  return (
    <video ref={ref} src={TRACK_SRC} muted playsInline preload="auto"
      data-om-exportable-video-play-start={s0}
      data-om-exportable-video-play-end={s1}
